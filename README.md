...
## Настройка RAID 5 из дисков

**RAID** - технология, благодаря которой несколько физических дисков объединяются в один логический, т.е. в случае поломки физ. диска, информация сохранится на остальных.

Есть несколько типов, самые популярные: 
- **RAID 0** - цельные данные дробятся на блоки и части и записываются на 2 и более диска. Если один из физ. носителей выйдет из строя, данные будут потеряны. Но при нет потери в дисковом пространстве.
- **RAID 1** - данные записываются на 2 и более диска сразу. Обрезает дисковое пространство в 2 раза.
- **RAID 5** - записывает файлы частями на все физ. диски и дополнительно записывает контрольную hash -сумму, которая называется **parity**, которая нужна для восстановления данных в случае поломки одного из физ. носителей. Около трети дискового пространства используется для parity.
- **RAID 10** - совмещенный вариант RAID 0 и RAID 1. Минимум 4 диска, должно быть кратно двум. Данные делятся на 2 группы и объединяются в RAID 1, которые уже делятся частями на каждый физ. диск, объединяются в RAID 0.

Мы используем **RAID 5**. 

Устанавливаем mdadm, если отсутствует:

    apt-get install -y mdadm
  
Смотрим утилитой `lsblk` все созданные диски:

![enter image description here](https://i.imgur.com/1QPvgdh.png)

  Мы будем объединять sdb,sdc,sdd по 1ГБ. Стираем данные суперблоков. Выдаст ошибку, если RAID не был ранее создан:
...
> - /dev/md0 - устройство RAID, которое появится после сборки (наш объединенный логический RAID диск)
> - -l 5 - уровень RAID
> - -n 3 - количество дисков, которое будет объединяться 
> -  /dev/sd{b,c,d} - указываем физ. носители, которые будут объединяться

![enter image description here](https://i.imgur.com/ioCkJQF.png)

Проверяем созданный логический RAID диск:

    lsblk
![enter image description here](https://i.imgur.com/Y743stw.png)
...
![enter image description here](https://i.imgur.com/y1qnfJd.png)
...
![enter image description here](https://i.imgur.com/DhGrrVb.png)
...
blkid | grep /dev/md0
```
![enter image description here](https://i.imgur.com/xtHgctK.png)

Добавляем **UUID**  в `/etc/fstab`:

    blkid | grep /dev/md0 >> /etc/fstab

Открываем  `fstab`  и приводим к следующему виду:

```
nano /etc/fstab
```
![enter image description here](https://i.imgur.com/U0ztjdO.png)

> - UUID - идентификатор раздела, который мы добавили с помощью команды выше
> - /mnt - каталог, в котором перемонтирован массив
